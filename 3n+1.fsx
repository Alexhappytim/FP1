...
module TailRecursive =
    let rec collatzLengthAcc (n: int64) (acc: int) : int =
        if n = 1L then
            ...
            collatzLengthAcc (n / 2L) (acc + 1)
        else
            collatzLengthAcc (3L * n + 1L) (acc + 1)
...
module NonTailRecursive =
    let rec collatzLength (n: int64) : int =
        if n = 1L then
            ...
            1 + collatzLength (n / 2L)
        else
            1 + collatzLength (3L * n + 1L)
...
module LoopSyntax =
    let collatzLength (n: int64) =
        ...
        while current <> 1L do
            length <- length + 1
            current <- if current % 2L = 0L then current / 2L else 3L * current + 1L
    ...
    let collatzSeqUnfold (n: int64) =
        ...
        |> Seq.unfold (fun state ->
            if state = 1L then
                ...
                None
            elif state % 2L = 0L then
                Some(state, state / 2L)
            else
                Some(state, 3L * state + 1L))
    
    let collatzLength n =
        collatzSeqUnfold n |> Seq.length
