...
module Power2Tests =
    ...
    let rec sumDigitsAcc (n: bigint) (acc: int) =
        if n = bigint.Zero then acc
        else sumDigitsAcc (n / bigint 10) (acc + int (n % bigint 10))
    
    let sumDigits n = sumDigitsAcc n 0
    
    // Обычная рекурсия
    let rec sumDigitsNonTail (n: bigint) =
        if n = bigint.Zero then 0
        else int (n % bigint 10) + sumDigitsNonTail (n / bigint 10)
    ...
    let runTests() =
        ...
        assertEqual 26 (sumDigits test15) "Sum of digits of 2^15 (tail recursive)" |> ignore
        assertEqual 26 (sumDigitsNonTail test15) "Sum of digits of 2^15 (non-tail recursive)" |> ignore
        assertEqual 26 (sumDigitsMap test15) "Sum of digits of 2^15 (map-based)" |> ignore
        
        // Тест на малых степенях
        assertEqual 1 (sumDigits (bigint.Pow(bigint 2, 0))) "Sum of digits of 2^0 = 1" |> ignore
        assertEqual 2 (sumDigits (bigint.Pow(bigint 2, 1))) "Sum of digits of 2^1 = 2" |> ignore
        assertEqual 4 (sumDigits (bigint.Pow(bigint 2, 2))) "Sum of digits of 2^2 = 4" |> ignore
        assertEqual 8 (sumDigits (bigint.Pow(bigint 2, 3))) "Sum of digits of 2^3 = 8" |> ignore
        assertEqual 7 (sumDigits (bigint.Pow(bigint 2, 4))) "Sum of digits of 2^4 = 16 -> 7" |> ignore
        ...
        let expected = 1366
        assertEqual expected (Pow2Solutions.TailRecursive.solve()) "Pow2 TailRecursive.solve = 1366" |> ignore
        assertEqual expected (Pow2Solutions.NonTailRecursive.solve()) "Pow2 NonTailRecursive.solve = 1366" |> ignore
        assertEqual expected (Pow2Solutions.Modular.solve()) "Pow2 Modular.solve = 1366" |> ignore
        assertEqual expected (Pow2Solutions.MapBased.solve()) "Pow2 MapBased.solve = 1366" |> ignore
        assertEqual expected (Pow2Solutions.LoopSyntax.solve()) "Pow2 LoopSyntax.solve = 1366" |> ignore
        assertEqual expected (Pow2Solutions.LazyInfinite.solve()) "Pow2 LazyInfinite.solve = 1366" |> ignore
...
module CollatzTests =
    ...
    let collatzSeq (n: int64) =
        let rec generate current =
            seq {
                yield current
                if current <> 1L then
                    if current % 2L = 0L then yield! generate (current / 2L)
                    else yield! generate (3L * current + 1L)
            }
        generate n
    ...
    let runTests() =
        ...
        assertEqual 10 (collatzLength 13L) "Collatz length of 13 (tail recursive)" |> ignore
        assertEqual 10 (collatzLengthNonTail 13L) "Collatz length of 13 (non-tail recursive)" |> ignore
        assertEqual 10 (collatzSeq 13L |> Seq.length) "Collatz length of 13 (sequence)" |> ignore
        ...
        for num in testNumbers do
            ...
            assertEqual tailResult nonTailResult (sprintf "Methods match for %d (tail vs non-tail)" num) |> ignore
            assertEqual tailResult seqResult (sprintf "Methods match for %d (tail vs seq)" num) |> ignore
        
        assertEqual 525 (collatzLength 837799L) "Collatz length of 837799" |> ignore

        let expectedLen = 525
        let n = 837799L
        assertEqual expectedLen (CollatzSolutions.TailRecursive.collatzLength n) "Collatz TailRecursive.collatzLength = 525" |> ignore
        assertEqual expectedLen (CollatzSolutions.NonTailRecursive.collatzLength n) "Collatz NonTailRecursive.collatzLength = 525" |> ignore
        assertEqual expectedLen (CollatzSolutions.Modular.collatzLength n) "Collatz Modular.collatzLength = 525" |> ignore
        assertEqual expectedLen (CollatzSolutions.MapBased.generateCollatzList n |> List.length) "Collatz MapBased length = 525" |> ignore
        assertEqual expectedLen (CollatzSolutions.LoopSyntax.collatzLength n) "Collatz LoopSyntax.collatzLength = 525" |> ignore
        assertEqual expectedLen (CollatzSolutions.LazyInfinite.collatzLength n) "Collatz LazyInfinite.collatzLength = 525" |> ignore
